def change_task(result):
    ...
    for item in result:
        new_item = {
            ...
            'single_bet_amount': item['single_bet_amount'],
        ...
        if change < 0:
            ...
            if item['bonus'] > 0:
                ...
                if item['wager'] >= item['up_point'] * 2 + item['bonus'] * 20:
                    # 流水打够的 返的就不用减
                    new_bonus = abs(change) * 0.15
                    # 充值金额为昨日亏损金额
                    new_up_point = change
                    # 等比缩单注金额
                    new_single_bet = new_up_point / item['up_point'] * item['single_bet_amount']
                else:
                    # 没有流水打够的 返的就要减
                    new_bonus = (change - item['bonus']) * 0.15
                    if new_bonus < 0:
                        new_bonus = 0
                    # 充值金额为昨日亏损金额
                    new_up_point = change
                ...
                new_item['single_bet_amount'] = new_single_bet
            else:
                # 玩家昨天没有拿到奖励
                new_bonus = change * 0.15
                # 充值金额为昨日亏损金额
                new_up_point = change
                # 等比缩单注金额
                new_single_bet = new_up_point / item['up_point'] * item['single_bet_amount']
